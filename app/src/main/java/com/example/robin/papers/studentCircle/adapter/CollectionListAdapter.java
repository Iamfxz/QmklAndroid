...
public class CollectionListAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup arg2) {
        ...
        if (convertView == null) {
            ...
            holder.list_img = convertView.findViewById(R.id.listuserimg);
            holder.username = convertView.findViewById(R.id.username);
            holder.dateTtime = convertView.findViewById(R.id.date_time);
            holder.fullText = convertView.findViewById(R.id.fullText);
            holder.usercontent = convertView.findViewById(R.id.usercontent);
        ...
        if(info.postInfo!=null){
            ...
            //请求该评论app使用者是否收藏
            RetrofitUtils.postIsCollect(context,postIsLikeRequest,position,CollectionListAdapter.class,holder.collection);

            holder.comment_count.setText(info.postInfo.getCommentNum()+"");//评论数


            //添加点赞监听
            holder.like.setOnClickListener(new LikeOnclick(context,CollectionListAdapter.class,holder.like_count,holder.like,null,position,postIsLikeRequest));
            holder.like_count.setOnClickListener(new LikeOnclick(context,CollectionListAdapter.class,holder.like_count,holder.like,null,position,postIsLikeRequest));
            holder.like_text.setOnClickListener(new LikeOnclick(context,CollectionListAdapter.class,holder.like_count,holder.like,null,position,postIsLikeRequest));

            //点击进入详情页监听
            holder.mixViewLayout.setOnClickListener(new UserContentOnclick(context,position,CollectionListAdapter.class));
            holder.usercontent.setOnClickListener(new UserContentOnclick(context,position,CollectionListAdapter.class));
            //添加发表回复监听
            holder.comment.setOnClickListener(new CommentOnclick(context,position,CollectionListAdapter.class));
            holder.comment_count.setOnClickListener(new CommentOnclick(context,position,CollectionListAdapter.class));
            holder.comment_text.setOnClickListener(new CommentOnclick(context,position,CollectionListAdapter.class));
            ...
            //收藏监听
            holder.collection.setOnClickListener(new CollectionOnclick(context, CollectionListAdapter.class, position, holder.collection));
        ...
        //原贴已删除，取消收藏
            ...
            Mixinfo mixinfo=data.get(position);
            mixinfo.is_collect=true;
            data.set(position,mixinfo);
            //显示此贴收藏时间
            holder.dateTtime.setText(info.collectionInfo.collectionListData.getCreateTime());//留言时间
            collectPost(position,holder.collection);
        }
        return convertView;
    }
    ...
    private void collectPost(int position, ImageView collection){
        String token= SharedPreferencesUtils.getStoredMessage(context,"token");
        PostIsLikeRequest postIsLikeRequest=new PostIsLikeRequest(token,data.get(position).collectionInfo.collectionListData.getPostId()+"");
        RetrofitUtils.postCollectPost(null,postIsLikeRequest,data.get(position),position,CollectionListAdapter.class,collection);
    }
